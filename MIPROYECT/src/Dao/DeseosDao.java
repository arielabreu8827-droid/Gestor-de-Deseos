...
public class DeseosDao {
    ...
    public List<Deseos> listar() throws SQLException {
        ...
        String sql = "SELECT nombre, precio, cantidad FROM deseos WHERE is_active = 1";
        try (Connection c = ConexionBD.conectar();
            ...
            while (r.next()) {
                ...
                l.add(new Deseos(nombre, precio, cantidad));
            }
        }
        return l;
    }
    ...
    public int insertar(Deseos e) throws SQLException {
        String sql = "INSERT INTO deseos (nombre, precio, cantidad) VALUES (?, ?, ?)";
        try (Connection c = ConexionBD.conectar();
    ...
    public int actualizar(int id, Deseos e) throws SQLException {
        String sql = "UPDATE deseos SET nombre = ?, precio = ?, cantidad = ? WHERE id = ?";
        try (Connection c = ConexionBD.conectar();
             PreparedStatement p = c.prepareStatement(sql)) {
    ...
    public int eliminar(int id) throws SQLException {
        String sql = "UPDATE deseos SET is_active = 0 WHERE id = ?";
        try (Connection c = ConexionBD.conectar();
             PreparedStatement p = c.prepareStatement(sql)) {
